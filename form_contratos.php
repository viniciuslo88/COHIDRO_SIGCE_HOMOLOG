...
function coh_values_differ($db_val, $form_val, $col_name) {
  ...
  if (preg_match('/Date|Data|Inicio|Fim|Assinatura/i', (string)$col_name)) {
    if (preg_match('~^(\d{2})/(\d{2})/(\d{4})$~', $v_form, $m)) $v_form = "{$m[3]}-{$m[2]}-{$m[1]}";
    $v_db   = substr($v_db, 0, 10);
    $v_form = substr($v_form, 0, 10);
  }

  if (preg_match('/Valor|Saldo|Percentual|Dias/i', (string)$col_name)) {
...
if (!function_exists('coh_pode_alterar')) {
  function coh_pode_alterar($created_at, $tem_permissao_nivel = true) {
    ...
    $timestamp_criacao = strtotime((string)$created_at);
...
/* ============================================================
   Auditoria de alterações de contrato (Gerenciamento)
   ✅ inclui detalhes_json (antes/depois) e garante coluna em bases antigas
============================================================ */
function ensure_contratos_log_schema(mysqli $conn): void {
  ...
  // garante coluna detalhes_json mesmo em tabelas antigas
  $has = false;
...
/**
 * ✅ LOG ROBUSTO: garante que detalhes_json sempre vire STRING (nunca NULL por “engolir array”)
 */
function coh_log_contrato_change(mysqli $conn, int $contrato_id, string $acao, string $detalhes = '', $detalhes_json = null): void {
  ...
  if (!empty($_SESSION['nome'])) {
    ...
    global $_USER;
    if (!empty($_USER['nome'])) $nome = (string)$_USER['nome'];
  }

  // ✅ normaliza detalhes_json em STRING sempre
  $dj = null;

  if ($detalhes_json === null) {
    $dj = json_encode(['campos'=>[], 'meta'=>['info'=>'sem detalhes_json']], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
  } elseif (is_string($detalhes_json)) {
    $dj = trim($detalhes_json);
  } else {
    $dj = json_encode($detalhes_json, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
  }

  if ($dj === null || $dj === '' || $dj === 'null' || $dj === false) {
    $dj = json_encode(['campos'=>[], 'meta'=>['erro'=>'json_vazio', 'msg'=>json_last_error_msg()]], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
  }

  if ($st = $conn->prepare("INSERT INTO emop_contratos_log (contrato_id,usuario_id,usuario_nome,diretoria,acao,detalhes,detalhes_json) VALUES (?,?,?,?,?,?,?)")) {
    $st->bind_param('iisssss', $contrato_id, $uid, $nome, $dir, $acao, $detalhes, $dj);
    if (!$st->execute()) {
      error_log('[COH_LOG] Falha ao inserir em emop_contratos_log: '.$st->error);
      error_log('[COH_LOG] dj_len='.(strlen((string)$dj)).' contrato_id='.$contrato_id.' acao='.$acao);
    }
    $st->close();
  } else {
    error_log('[COH_LOG] Prepare falhou: '.$conn->error);
  }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($action === 'salvar' || $action === 'salvar_direto') {
    ...
    $diff_campos_json = []; // ✅ antes/depois por campo (para detalhes_json)
    ...
    try {
      if ($contrato_id > 0 && !empty($cols)) {
        ...
        foreach ($cols as $c) {
          ...
          if ($isDateLike) {
            if ($v === '') $v = null;
            elseif (preg_match('~^(\d{2})/(\d{2})/(\d{4})$~', (string)$v, $mData)) $v = "{$mData[3]}-{$mData[2]}-{$mData[1]}";
          }
          ...
          $oldVal = array_key_exists($c, $rowNow) ? $rowNow[$c] : null;

          if (coh_values_differ($oldVal, $v, $c)) {
            $nomeCampo = str_replace('_', ' ', $c);
            $alteracoes_realizadas[] = "Campo {$nomeCampo} atualizado.";

            // ✅ registra antes/depois para o histórico do GERENCIAMENTO
            $diff_campos_json[$c] = [
              'antes'  => $oldVal,
              'depois' => $v,
            ];
      ...
      // Inserts (medições/aditivos/reajustes)
      ...
      if ($m) {
        coh_insert_medicoes_from_array($conn,$contrato_id,$m);
        $alteracoes_realizadas[] = count($m) . " Novas Medições.";
        $diff_campos_json['MEDICOES'] = ['antes' => null, 'depois' => (count($m) . ' novas medições')];
      }
      if ($a) {
        coh_insert_aditivos_from_array($conn,$contrato_id,$a);
        $alteracoes_realizadas[] = count($a) . " Novos Aditivos.";
        $diff_campos_json['ADITIVOS'] = ['antes' => null, 'depois' => (count($a) . ' novos aditivos')];
      }
      if ($r) {
        coh_insert_reajustes_from_array($conn,$contrato_id,$r);
        $alteracoes_realizadas[] = count($r) . " Novos Reajustes.";
        $diff_campos_json['REAJUSTES'] = ['antes' => null, 'depois' => (count($r) . ' novos reajustes')];
      }
      ...
      if ($contrato_id > 0) {
        ...
        if ($stB = $conn->prepare("SELECT COALESCE(Valor_Do_Contrato, 0) FROM emop_contratos WHERE id=?")) {
          $stB->bind_param('i', $contrato_id);
          $stB->execute();
          $stB->bind_result($baseContrato);
          $stB->fetch();
          $stB->close();
        }
        ...
        if (!empty($ult)) {
          ...
          // ✅ CORRIGE o parse error antigo e garante acumulado
          if ($acum <= 0 && isset($last['acumulado_rs']) && is_numeric($last['acumulado_rs'])) {
            $acum = (float)$last['acumulado_rs'];
          }

          $perc_exec = $vtc > 0 ? ($acum / $vtc) * 100.0 : 0.0;

          $conn->query("UPDATE emop_contratos
                        SET Valor_Liquidado_Na_Medicao_RS = '{$vlr_med}',
                            Valor_Liquidado_Acumulado = '{$acum}',
                            Percentual_Executado = '{$perc_exec}'
                        WHERE id = {$contrato_id}");
        }

        $conn->query("UPDATE emop_contratos
                      SET Aditivos_RS = '{$totalAditivos}',
                          Contrato_Apos_Aditivo_Valor_Total_RS = '{$valorAposAditivos}',
                          Valor_Dos_Reajustes_RS = '{$totalReajustes}',
                          Valor_Total_Do_Contrato_Novo = '{$valorTotalAtualContrato}'
                      WHERE id = {$contrato_id}");
      }
      ...
      if (!empty($alteracoes_realizadas)) {
        ...
        // ✅ gera JSON como STRING antes de gravar (pra nunca ficar NULL)
        $detalhes_json_arr = [
          'campos' => $diff_campos_json,
          'meta'   => [
            'medicoes_inseridas'  => is_array($m) ? count($m) : 0,
            'aditivos_inseridos'  => is_array($a) ? count($a) : 0,
            'reajustes_inseridos' => is_array($r) ? count($r) : 0,
          ],
        ];

        $detalhes_json_str = json_encode($detalhes_json_arr, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
        if ($detalhes_json_str === false || $detalhes_json_str === null || trim((string)$detalhes_json_str) === '') {
          $detalhes_json_str = json_encode(
            ['campos'=>[], 'meta'=>['erro'=>'json_encode_failed', 'msg'=>json_last_error_msg()]],
            JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES
          );
        }

        coh_log_contrato_change($conn, $contrato_id, 'SALVAR_DIRETO', $detalhes_log, $detalhes_json_str);
      }
  ...
  if ($action === 'solicitar_aprovacao') {
    ...
    if (!empty($novas_medicoes) && is_array($novas_medicoes)) {
      ...
      foreach ($novas_medicoes as $m) {
        ...
        if (!empty($m['data_medicao'])) {
          $d = $m['data_medicao'];
          if (preg_match('/^\d{4}-\d{2}-\d{2}$/', (string)$d)) $d = date('d/m/Y', strtotime($d));
          $partes[] = "Data: {$d}";
        }
        if (!empty($m['valor_rs']))      $partes[] = "Valor: {$m['valor_rs']}";
        if (!empty($m['acumulado_rs']))  $partes[] = "Acumulado: {$m['acumulado_rs']}";
        if (!empty($m['percentual']))    $partes[] = "%: {$m['percentual']}";
        if (!empty($m['observacao']))    $partes[] = "Obs: {$m['observacao']}";

        if ($partes) $linhas[] = implode(' | ', $partes);
      }
      if ($linhas) $campos['NOVAS_MEDICOES'] = implode("\n", $linhas);
    }
    ...
    if (!empty($novos_aditivos) && is_array($novos_aditivos)) {
      ...
      foreach ($novos_aditivos as $a) {
        ...
        if (!empty($a['numero_aditivo'])) $partes[] = "Número: {$a['numero_aditivo']}";
        if (!empty($a['data'])) {
          $d = $a['data'];
          if (preg_match('/^\d{4}-\d{2}-\d{2}$/', (string)$d)) $d = date('d/m/Y', strtotime($d));
          $partes[] = "Data: {$d}";
        }
        if (!empty($a['tipo']))                 $partes[] = "Tipo: {$a['tipo']}";
        if (!empty($a['valor_aditivo_total']))  $partes[] = "Valor: {$a['valor_aditivo_total']}";
        if (!empty($a['valor_total_apos_aditivo'])) $partes[] = "Total após: {$a['valor_total_apos_aditivo']}";
        if (!empty($a['novo_prazo']))           $partes[] = "Novo prazo: {$a['novo_prazo']}";
        if (!empty($a['observacao']))           $partes[] = "Obs: {$a['observacao']}";

        if ($partes) $linhas[] = implode(' | ', $partes);
      }
      if ($linhas) $campos['NOVOS_ADITIVOS'] = implode("\n", $linhas);
    }
    ...
    if (!empty($novos_reajustes) && is_array($novos_reajustes)) {
      ...
      foreach ($novos_reajustes as $rj) {
        ...
        if (!empty($rj['percentual']))                $partes[] = "Percentual: {$rj['percentual']}";
        if (!empty($rj['valor_total_apos_reajuste'])) $partes[] = "Total após: {$rj['valor_total_apos_reajuste']}";
        if (!empty($rj['observacao']))                $partes[] = "Obs: {$rj['observacao']}";

        if ($partes) $linhas[] = implode(' | ', $partes);
      }
      if ($linhas) $campos['NOVOS_REAJUSTES'] = implode("\n", $linhas);
    }
    ...
    // 4) Grava na inbox do Coordenador
    ...
    if ($review_id_post > 0) {
      $st = $conn->prepare("UPDATE coordenador_inbox SET payload_json=?, status='PENDENTE', created_at=NOW(), updated_at=NULL WHERE id=? AND fiscal_id=?");
      if ($st) $st->bind_param("sii", $json, $review_id_post, $fiscal_id);
    } else {
      $st = $conn->prepare("INSERT INTO coordenador_inbox(contrato_id,diretoria,fiscal_id,payload_json,status) VALUES (?,?,?,?, 'PENDENTE')");
      if ($st) $st->bind_param("isis", $contrato_id, $dir, $fiscal_id, $json);
    }
...
$draft_lists = ['medicoes'=>[], 'aditivos'=>[], 'reajustes'=>[]];
$lastLog = null;
...
if ($contrato_id > 0) {
  ensure_contratos_log_schema($conn);
  if ($st = $conn->prepare("SELECT contrato_id, usuario_nome, diretoria, acao, detalhes, detalhes_json, created_at
    ...
    $st->execute();
    $rsLog = $st->get_result();
    if ($rsLog && $rsLog->num_rows) $lastLog = $rsLog->fetch_assoc();
    $st->close();
  }
}
...
if (!$HIDE_FORM && ($contrato_id || $is_new)) {
  ...
  if ($contrato_id > 0 && !empty($lastLog)) {
    ...
    if (!empty($lastLog['created_at'])) {
      ...
      if ($ts) $dtFmt = date('d/m/Y \à\s H:i', $ts);
    }

    // ✅ pega "último campo" preferencialmente do detalhes_json (antes/depois)
    $ultimoCampo = '';
    $dj = null;
    if (!empty($lastLog['detalhes_json'])) {
      $dj = json_decode((string)$lastLog['detalhes_json'], true);
    }
    if (is_array($dj) && !empty($dj['campos']) && is_array($dj['campos'])) {
      $keys = array_keys($dj['campos']);
      if ($keys) {
        $kLast = (string)end($keys);
        $ultimoCampo = str_replace('_', ' ', $kLast);
      }
    }

    // fallback antigo: usa última linha de "detalhes"
    if ($ultimoCampo === '' && !empty($lastLog['detalhes'])) {
...
// --- Injeta o ID da revisão no formulário HTML ---
