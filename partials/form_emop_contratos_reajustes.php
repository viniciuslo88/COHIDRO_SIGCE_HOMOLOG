...
if (!defined('COH_DRAFT_INPUTS')) {
  ...
  echo '<input type="hidden" name="novas_medicoes_json" id="novas_medicoes_json" value="">';
  echo '<input type="hidden" name="novos_aditivos_json" id="novos_aditivos_json" value="">';
  echo '<input type="hidden" name="novos_reajustes_json" id="novos_reajustes_json" value="">';
}
...
if (!defined('COH_DRAFT_JS')) {
  ...
  window.cohAddReajuste=function(p){ var l='Reajuste '+(p.data_base||''); var d='Perc: '+(p.percentual||''); COH.draft.reajustes.push(Object.assign({_label:l,_desc:d}, p)); cohSetHiddenDraft(); cohRenderDraft('draft-list-reajustes', COH.draft.reajustes); };
  </script>
  <?php
}
...
<button type="button" class="btn btn-outline-primary mb-3" data-bs-toggle="modal" data-bs-target="#modalReajuste">+ Adicionar Reajuste</button>
        ...
        <?php foreach ($__reajustes_salvos as $r): ?>
          <?php 
             $pode_mexer = function_exists('coh_pode_alterar') 
                           ? coh_pode_alterar($r['created_at'] ?? null, $tem_permissao_geral) 
                           : false;
          ...
          <tr>
            ...
            <td><?= coh_pct($r['percentual'] ?? 0) ?></td>
            <td><?= coh_brl($r['valor_total_apos_reajuste'] ?? 0) ?></td>
            <td class="text-end" style="min-width: 140px;">
                <?php if ($pode_mexer): ?>
                    <div class="btn-group btn-group-sm mb-1">
                        <button type="button" class="btn btn-outline-secondary" 
                                onclick='cohEditDbItem("reajuste", <?= json_encode($r) ?>)' title="Editar">
                            <i class="bi bi-pencil"></i>
                        </button>
                        <button type="button" class="btn btn-outline-danger" 
                                onclick="cohDeleteDbItem('reajuste', <?= $r['id'] ?>)" title="Excluir">
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
    ...
    <div class="modal-body">
      <div class="alert alert-light border py-1 px-2 mb-3 small text-muted">
        <i class="bi bi-info-circle"></i> Base de cálculo: <strong><?= e(coh_brl($valor_base_calculo)) ?></strong> (<?= e($origem_base) ?>)
      </div>
      ...
      <div class="row g-3">
        ...
        <div class="col-md-12">
          <label>Valor Total Após Reajuste (R$)</label>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if (!inpPercentual || !inpTotal || !baseHidden) return;
    ...
    inpPercentual.addEventListener('input', function() {
        ...
        const novoTotal = baseVal * (1 + (perc / 100));
        inpTotal.value = fmtBR(novoTotal);
    });
