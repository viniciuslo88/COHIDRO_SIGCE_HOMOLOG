...
// ===== Variáveis de Escopo e Permissão =====
$__cid = (int)($id ?? ($row['id'] ?? 0));

// Define a permissão baseada no nível do usuário (vem do form_contratos.php)
// Se $user_level não existir, assume false por segurança, ou true se for ambiente de dev
$tem_permissao_geral = isset($user_level) ? ($user_level >= 2) : true;

// Valor base do contrato (para cálculos)
...
if (isset($row) && is_array($row) && isset($row['Valor_Do_Contrato']) && $row['Valor_Do_Contrato'] !== '') {
  ...
  if ($raw !== '') {
    $raw = str_replace(['.', ','], ['', '.'], $raw); // Remove ponto milhar, troca virgula decimal
    $valor_base_contrato = (float)$raw;
  }
}

// ===== Campos hidden compartilhados para Rascunho =====
...
// ===== Script de rascunho global (Gerenciador JS) =====
if (!defined('COH_DRAFT_JS')) {
  ...
  // Funções de adicionar ao rascunho
  ...
  window.cohAddAditivo = function(p){
    ...
    var d='Tipo: '+(p.tipo||'')+' · Valor: '+(p.valor_aditivo_total||'')+' · Total após: '+(p.valor_total_apos_aditivo||'');
  ...
  window.cohAddReajuste = function(p){
    ...
    var d='Percentual: '+(p.percentual||'')+' · Total após: '+(p.valor_total_apos_reajuste||'');
...
// ===== BUSCA ADITIVOS SALVOS NO BANCO =====
...
if ($__cid > 0) {
  ...
  $sqlAd = "SELECT id, contrato_id, valor_aditivo_total, novo_prazo, valor_total_apos_aditivo, numero_aditivo, tipo, created_at, observacao
            FROM emop_aditivos
            WHERE contrato_id = ?
  ...
  if ($st = $conn->prepare($sqlAd)) {
    ...
    $st->execute();
    $rs = $st->get_result();
    $prev_acum = 0.0; // Se quiser calcular dinamicamente o anterior
    ...
    while ($r = $rs->fetch_assoc()) {
      ...
      $__aditivos_salvos[] = [
        'id'                     => $r['id'], // Importante para exclusão
    ...
    <table class="table table-sm table-hover align-middle">
      <thead class="table-light">
        <tr>
          <th>Data</th>
          <th>Número/Tipo</th>
          <th>Valor do aditivo (R$)</th>
          <th>Aditivo acumulado (R$)</th>
          <th>Novo prazo</th>
      ...
      <tbody>
        <?php foreach ($__aditivos_salvos as $a): ?>
          <?php 
             ...
             // Verifica se a função existe (está no pai) e passa a permissão de nível
            ...
            <td class="text-end">
                <?php if ($pode_mexer): ?>
                    <button type="button" class="btn btn-sm btn-outline-secondary" 
                            onclick='cohEditDbItem("aditivo", <?= json_encode($a) ?>)' title="Editar (Cria novo)">
...
<div class="modal fade" id="modalAditivo" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content">
    <div class="modal-header">
      <h5 class="modal-title">Novo Aditivo (Rascunho)</h5>
    ...
    <div class="modal-body">
      <div class="row g-3">
        <div class="col-md-3">
          ...
          <input type="text" name="numero_aditivo" class="form-control">
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-6">
          <label class="form-label">Valor do Aditivo (R$)</label>
          <input type="text" name="valor_aditivo_total" class="form-control" id="adt_valor" placeholder="0,00">
        </div>
        <div class="col-md-6">
          <label class="form-label">Valor Total após Aditivo (R$)</label>
          <input type="text" name="valor_total_apos_aditivo" class="form-control" id="adt_total" placeholder="0,00">
        </div>
        <div class="col-12">
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // Pega o valor base do contrato (vindo do input hidden PHP)
  var baseInput = document.getElementById('valor_base_contrato');
  var valorBaseContrato = 0;
  if (baseInput) {
    valorBaseContrato = parseFloat(String(baseInput.value)) || 0;
  }
